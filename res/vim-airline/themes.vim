  ...
  function! airline#themes#patch(palette)
    for mode in keys(a:palette)
      ...
      if !has_key(a:palette[mode], 'airline_warning')
        let a:palette[mode]['airline_warning'] = [ '#000000', '#88aea3', 232, 166 ]
      endif
      if !has_key(a:palette[mode], 'airline_error')
        let a:palette[mode]['airline_error'] = [ '#000000', '#e1dbcd', 232, 160 ]
      endif
  ...
  def airline#themes#patch(palette: dict<any>): void
    for mode in keys(palette)
      ...
      if !has_key(palette[mode], 'airline_warning')
        extend(palette[mode], {airline_warning: [ '#000000', '#88aea3', '232', '166' ]})
      endif
      if !has_key(palette[mode], 'airline_error')
        extend(palette[mode], {airline_error: [ '#000000', '#e1dbcd', '232', '160' ]})
      endif
